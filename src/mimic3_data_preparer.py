...
flags.DEFINE_string("postgres", default="dbname=mimic user=postgres password=shilong",
...
flags.DEFINE_string("mimic_iii_hadm_ids_file_dir", default="your_path", help="directory of Hadm ids files for datasets")
flags.DEFINE_string("data_output_dir", default="your_path",
...
flags.DEFINE_bool("segment_text", default=True, help="if segment text into meaningful chunks")
...
def get_text_labels(admission_id, cur, top_n_labels):
    ...
    select_statement = "select t1.icd9_code as icd9_code, t2.long_title as long_title " \
                       ...
                       "on t1.icd9_code=t2.icd9_code WHERE hadm_id={} ORDER BY seq_num".format(admission_id)
    cur.execute(select_statement)
    diag_chapter_labels, diag_three_character_labels, diag_full_labels, diag_labels_dictionary = process_codes(cur, True, top_n_labels)

    select_statement = "select t1.icd9_code as icd9_code, t2.long_title as long_title " \
                       "FROM procedures_icd as t1 left join d_icd_procedures as t2 " \
                       "on t1.icd9_code=t2.icd9_code WHERE hadm_id={} ORDER BY seq_num".format(admission_id)
    cur.execute(select_statement)
    proc_chapter_labels, proc_three_character_labels, proc_full_labels, proc_labels_dictionary = process_codes(cur, False, top_n_labels)
