...
from utils import initial_code_title_vectors
...
class CodingModel(torch.nn.Module):
    def __init__(self, coding_model_config, args):
        ...
        self._init_linear_weights(mean=self.coding_model_config.linear_init_mean,
                                  std=self.coding_model_config.linear_init_std)

        # leave
        self.leaves_emb = nn.Embedding(1499, 20, padding_idx=0, sparse=False)
        self.leaves_dropout = nn.Dropout(0.3)
        self.W_q = nn.Linear(768, 128)
        self.k = nn.Linear(128, 50)
        self.convertor = nn.Linear(788, 768)
        self.nor_layer = nn.LayerNorm(768)

    def _init_linear_weights(self, mean, std):
        torch.nn.init.normal_(self.classifier_layer.weight, mean, std)

    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, targets=None, leaves=None):
        transformer_output = []
        ...
        leaf_emds = self.leaves_emb(leaves)
        leaf_emds = self.leaves_dropout(leaf_emds)
        q = self.W_q(l4_dropout)
        att = self.k(q)
        x = att.softmax(2) @ leaf_emds
        x = self.leaves_dropout(x)
        x = torch.cat((l4_dropout, x), dim=2)
        x = self.convertor(x)
        x = self.nor_layer(x)
        l4_dropout = x.tanh()
        ...
        loss_fct_local = FocalLoss(0.25, 2)
        loss = loss_fct(logits, targets)
        loss_focal = loss_fct_local(logits, targets)
        loss = 0.7*loss + 0.3*loss_focal
        return {
            "loss": loss,
            "logits": logits,
            "preds": preds,
